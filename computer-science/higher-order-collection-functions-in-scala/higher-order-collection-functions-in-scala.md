...
- [Higher-order functions](#higher-order-list-functions)
	- [map](#1-map)
	- [flatMap](#2-flatmap)
	- [filter](#3-filter)
	- [foldLeft](#4-foldleft)
	- [foldRight](#5-foldright)
	- [reduceLeft](#6-reduceleft)
	- [reduceRight](#7-reduceright)
	- [scanLeft](#8-scanleft)
	- [scanRight](#9-scanright)
	- [zip](#10-zip)
	- [zipWithIndex](#11-zipwithindex)
	- [forall](#12-forall)
	- [exists](#13-exists)
	- [groupBy](#14-groupby)
	- [sorted](#15-sorted)
	- [sortBy](#16-sortby)
	- [sortWith](#17-sortwith)
	- [minBy & maxBy](#18-minby--maxby)
	- [Creating a generic higher-order collection function](#creating-a-generic-higher-order-collection-function)
	- [Chaining functions](#chaining-functions)
		- [mapReduce](#1-mapreduce)
		- [zipFlattenOrderBy](#2-zipflattenorderby)
...
- **Case 1:** When the list is empty (*`Nil`*), return the most basic unit of the return type we're trying to produce.
...
## 15. sorted
`sorted` is used for sorting collections. The sort is stable, meaning that equal elements (*as determined by `ord.compare`*) appear in the same order in the sorted sequence as in the original. 
...
Fluent API is a design pattern that enables code to be written in a way that resembles natural language or a domain-specific language (*DSL*). It allows for chaining multiple operations or method calls together concisely and readably.
